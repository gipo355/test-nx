...
module.exports = {
  ...
  //     assetModuleFilename: 'assets/[name][ext][query]',
  // }, // #endregion
  // below for the test if statement
  // output,
  ...
  plugins: [
    ...
    // * clear old files
    /*
     * commented out here because in natours we are using a double config, the second
     * config would clear the first one, only want this in the first config
     */
  ...
  module: {
    rules: [
      ...
      // * needed to be able to require and build images in js
      {
        ...
        // test: /\.(png|jpe?g|gif|mp3|mp4|svg|json|txt)$/,
      ...
      /**
       * PUG
       * this is conflicting with htmlwebpackplugin.
       * if i use pug laoder, the template gets converted to html
       */
      // {
      //   test: /\.pug$/,
      //   exclude: /node_modules/,
  ...
  //   ! optimizers #region[4]
  //   persist cache in node modules cache
  cache: {
    type: 'filesystem',
  },
  // optimization: {
  //   // runtimeChunk: 'single', // in webpack caching guide to extract runtime IMP: conflicts with tests ( emits runtime.spec file)
  //   // usedExports: false,
  //   // sideEffects: false,
