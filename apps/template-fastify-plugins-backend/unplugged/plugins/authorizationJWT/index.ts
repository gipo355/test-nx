 ...
 * can use local auth(email+pw), oauth2.
 *
 * oauth2 helpers include fastify-oauth2(simple-oauth), fastify-auth0-verify, fastify-passport,
 * auth.js, lucia
 *
 * lucia, authjs and fastify-oauth2 provide helpers to login with oauth2 (authjs allows jwts)
 * lucia and authjs did a similar implementation to mine
 * authjs instead of self-made refresh tokens, uses the refresh token from oauth2
 *
 ...
 *
 * for storing tokens on the client we use cookies
 * to send them we use cookies [sveltekit proxy: Authorization header or body]
 * (jwts or session) and redis
 * only over https
 *
 *
...
const plugin: FastifyPluginAsync = async (fastify) => {
   ...
   * with redis store
   * NOTE: fastify-session can only be used with cookies
   */
  ...
  await fastify.register(fastifySession, {
    ...
    cookie: {
      ...
      path: '/',
      httpOnly: true,
      expires: new Date(Date.now() + config.SESSION_COOKIE_EXPIRATION),
    },
    store: redisStore,
  });
...
declare module 'fastify' {
  ...
  // for fastify-session
  interface Session extends TAccessTokenPayload {
    userId: string;
