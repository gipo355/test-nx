...
class APIFeaturesJonas {
  ...
  filter() {
    ...
    // eslint-disable-next-line no-restricted-syntax, security/detect-object-injection, @typescript-eslint/no-dynamic-delete
  ...
  limitFields() {
    ...
    if (queryOBJ.fields) {
      ...
      this.mongooseQuery = this.mongooseQuery.select(fieldsArguments);
  ...
  // we pass the query param because we need the origina object for the count
  // in my implementation, i didn't need it
  paginate() {
    const queryOBJ = { ...this.queryStringObj };
    /**
     * ! ## pagination
    ...
    // }
    // eslint-disable-next-line no-void
    this.mongooseQuery = this.mongooseQuery.skip(skip).limit(limitPerPage);
    // eslint-disable-next-line no-void
    ...
    // ! jonas removes it
    // if (queryOBJ.page) {
    //     const numberTours = await query.countDocuments();
